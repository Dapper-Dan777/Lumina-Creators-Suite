...
function getN8nEmbedUrl() {
  return `http://localhost:${n8nEmbedPort}`;
}
...
app.get('/api/n8n/status', async (req, res) => {
  if (!n8nBaseUrl) {
    return res.json({
      enabled: false,
      connected: false,
      message: 'N8N_BASE_URL ist nicht gesetzt. Starte n8n in Docker und trage die URL in .env ein.',
    });
  }
...
/** Read-only live stats for dashboard — fails gracefully if OF account not linked yet. */
...
/** Match all creators to OnlyFansAPI accounts (handle / URL) — no .env changes needed. */
  ...
  try {
    ...
    res.json({
      ...
      message: result.linked.length
        ...
        : 'Keine passenden Creator gefunden — Handle oder OnlyFans-URL muss zum OF-Account passen',
    });
  } catch (error) {
    console.error(error);
    res.status(500).json({ ok: false, error: error.message });
...
/** Sync live subscriber count from all linked OnlyFans accounts. */
  ...
  try {
    ...
    res.json({
      ...
      message: result.linked.length
        ...
        : 'Keine Creator verknüpft — Handle @username oder onlyfans.com/username beim Anlegen verwenden',
    });
  } catch (error) {
    console.error(error);
    res.status(500).json({ ok: false, error: error.message });
...
/** Sync OnlyFans chats into assisted inbox (read-only, all linked accounts). */
  ...
  try {
    ...
    if (!accounts.length) {
      return res.status(400).json({
        ok: false,
        error: 'Kein authentifizierter OnlyFans-Account — unter app.onlyfansapi.com verbinden',
      });
    }
    ...
    const filterHint =
      filter === 'unread'
        ? ' — keine ungelesenen Chats? Nutze filter "recent" oder "all"'
        : '';
    ...
    res.json({
      ...
      message: conversations.length
        ? `${conversations.length} Chat(s) von ${accountsSynced.length} OF-Account(s) geladen`
        : `Keine Chats gefunden${filterHint}`,
    });
  } catch (error) {
    console.error(error);
    res.status(500).json({ ok: false, error: error.message });
...
/** Send a chat message via OnlyFansAPI (manual send from Lumina inbox). */
...
/** Proxy OF CDN media for chat thumbnails (avoids hotlink/CORS issues). */
